...
export async function DELETE(req: Request, context: any) {
  try {
    ...
    const id = params?.id || (() => {
      try { return new URL(req.url).pathname.split('/').pop(); } catch { return undefined; }
    })();
...
export async function PATCH(req: Request, context: any) {
  try {
    ...
    const { data, error } = await supabase.from("hero_images").update(body).eq("id", id).select().single();
    if (error) throw error;
    return NextResponse.json({ success: true, data });
  } catch (error: any) {
    console.error(error);
